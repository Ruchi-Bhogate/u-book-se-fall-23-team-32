...
function CartPage() {
  ...
  const handleRemoveItem = async (itemId) => {
    ...
    try {
      await axios.delete(
        `https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/cartview/remove-cart-item/${itemId}`,
        {
  ...
  const updateItemInCart = async (updatedItem) => {
    ...
    try {
      ...
      await axios.put(
        `https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/cartview/update-cart-item`,
        updatedItem,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const calculateTotal = (items) => {
    return items
      .reduce((acc, item) => acc + item.price_per_day * item.days, 0)
      .toFixed(2);
  };
