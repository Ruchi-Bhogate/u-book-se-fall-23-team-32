...
function Forgot() {
  ...
  const fetchSecurityQuestions = async () => {
    try {
      ...
      //const response = await axios.post('https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/get-security-questions', { email });
      const response = await axios.post(
        "https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/get-security-questions",
        { email }
      );
      ...
      if (questionsObject) {
        ...
        console.error(
          "Questions object is missing or undefined in the response."
        );
      }
    } catch (error) {
  ...
  const handleAnswerSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/verify-answers",
        {
          email,
          answers,
        }
  ...
  const handleEmailSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/Forgot", {
        email,
      });
      console.log(response.data);
    } catch (error) {
  ...
  return (
    <div className="forgot-container">
      <form
        onSubmit={method === "email" ? handleEmailSubmit : handleAnswerSubmit}
        ...
        <div className="button-group">
          {method ? (
            <button type="submit" className="button">
              {method === "email" ? "Send Email" : "Submit Answers"}
            </button>
          ) : (
            <>
              <button
