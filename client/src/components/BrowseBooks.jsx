...
function BrowseBooksPage() {
    ...
    const fetchBooks = async () => {
      try {
        const response = await axios.get(
          "https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/userview/browsebooks"
        );
  ...
  useEffect(() => {
    const results = books.filter((book) => {
      ...
      const matchesCost =
        book.price_per_day >= minCost &&
        (maxCost === Infinity || book.price_per_day <= maxCost);

      // Check if the book matches the genre filter, if a genre is selected.
      const matchesGenre = selectedGenres
        ? book.genre === selectedGenres
        : true;

      // Check if the book matches the publication year filter, if a year is set.
      const matchesPublishedYear = publishedYear
        ? book.publication_year >= publishedYear
        : true;

      return (
        matchesSearchTerm && matchesCost && matchesGenre && matchesPublishedYear
      );
    });
