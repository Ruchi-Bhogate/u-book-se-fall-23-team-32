    ...
    const fetchBooks = async () => {
      try {
        const response = await axios.get(
          "https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/rentedbooks/getbooks",
          {
            headers: {
  ...
  const handleReturnBook = async (rentedBookId) => {
    try {
      const response = await axios.put(
        `https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/rentedbooks/update-status/${rentedBookId}`,
        { status: "returned" },
        {
      ...
      if (response.status === 200) {
        ...
        setRentedBooks((prevState) => ({
          ...prevState,
          current: prevState.current.filter(
            (book) => book._id !== rentedBookId
          ),
  ...
  const handleReviewSubmit = async (e, ownerId, rentedBookId) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://u-book-se-fall-23-team-32-hm4hr39j8-ruchis-projects-419a70ff.vercel.app/rentedbooks/review/${ownerId}`,
        {
          rating: rating.value,
      ...
      if (response.status === 200) {
        ...
        setRentedBooks((prevState) => ({
          ...prevState,
          pendingReview: prevState.pendingReview.filter(
            (book) => book._id !== rentedBookId
          ),
        }));
      }
      ...
      <div>
        ...
        <div className="rented-books">
          ...
          <div className="filter-buttons">
            <button onClick={() => setSelectedFilter("current")}>
              Current Rentals
            </button>
            <button onClick={() => setSelectedFilter("past")}>
              Past Rentals
            </button>
            <button onClick={() => setSelectedFilter("pendingReview")}>
              Pending Review
            </button>
          </div>
              ...
              <ul>
                {rentedBooks.current.map((book) => (
                  <li key={book._id}>
                    ...
                    <button onClick={() => handleReturnBook(book._id)}>
                      Return
                    </button>
                    <button
                      onClick={() => {
              ...
              <ul>
                {rentedBooks.pendingReview.map((book) => (
                  <li key={book._id}>
                    ...
                    <form
                      onSubmit={(e) =>
                        handleReviewSubmit(e, book.owner_user_id, book._id)
                      }
                    >
                      <input
                        type="number"
                      ...
                      <textarea
                        name="review"
                        required
                        placeholder="Write a review about the owner"
                      ></textarea>
